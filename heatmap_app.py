############################################################
# RNA-seq Heatmap Generator – Prekovic Lab (final version)
# Works with:
...
The app:
- Skips featureCounts header comments automatically  
- Detects *Geneid* and *Gene_Symbol*  
- Reads the first unnamed column in the annotation as sample names  
- Normalizes (`log2(counts+1)` or z-score)  
- Lets you customize palettes, labels, and figure size  
...
if expr_file and annot_file:
    try:
        ...
        expr = pd.read_csv(
            expr_file,
            sep="\t",
            comment="#",          # skip featureCounts header line
            low_memory=False,
            encoding="utf-8-sig"
        )
        ...
        if "gene_symbol" in lower_cols:
            sym_col = expr.columns[lower_cols.index("gene_symbol")]
            id_col = expr.columns[lower_cols.index("geneid")] if "geneid" in lower_cols else sym_col
